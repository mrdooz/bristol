  ...
  , _resizeFlags(0)
{
  ...
  _eventManager->RegisterHandler(Event::TextEntered, bind(&VirtualWindowManager::GenericHandler, this, _1));
...
bool VirtualWindowManager::OnMouseMoved(const Event& event)
{
  ...
  if (_focusWindow)
  {
    uint32_t mask    = _focusWindow->PointInsideBorder(pos);
...
bool VirtualWindowManager::OnMouseButtonPressed(const Event& event)
{
  ...
  for (VirtualWindow* window : _windows)
  {
    ...
    uint32_t mask = window->PointInsideBorder(pos);
...
//-----------------------------------------------------------------------------
uint32_t VirtualWindowManager::RegisterHandler(
...
//-----------------------------------------------------------------------------
void VirtualWindowManager::UnregisterHandler(uint32_t handle)
{
